...
export default function ProfilePage() {
  ...
  async function saveProfile() {
    ...
    await supabase.from('clients').update({ store_name: profile.store_name, phone: profile.phone })
    ...
    <div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div>
            <label style={{ fontSize: 12, color: '#6b7280', marginBottom: 4, display: 'block' }}>
              {lang === 'kz' ? 'Телефон номері' : 'Номер телефона'}
            </label>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <input type="password"
            placeholder={lang === 'kz' ? 'Жаңа пароль' : 'Новый пароль'}
            value={newPassword} onChange={e => setNewPassword(e.target.value)} />
          <input type="password"
            placeholder={lang === 'kz' ? 'Жаңа паролді қайталаңыз' : 'Повторите новый пароль'}
